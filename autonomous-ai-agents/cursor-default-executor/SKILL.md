...
# 🔴 铁律：Cursor 优先执行引擎（2026-08-19 升级版：任务启动 Gate + 无豁免）

## 🚦 任务启动 Gate（每次收到任务第一件事，动手前必须过）

```
收到任务 → 先分类，再动手：

┌─ 图片/OCR/截图/PDF识别 ──→ 1.派 Cursor（传图片/PDF路径，Cursor原生多模态）
│                             2.Cursor不可用 → 硅基流动千问直调（auxiliary.vision, Qwen3-VL）
│                             3.再不可用 → Firecrawl/本地OCR
│                             ❌ 禁止直接采信 Telegram 平台自动附带的内置描述文本
├─ 执行类：写文件/改代码/JSON数据写入(status.json/tasks/active.json等)/
│         待办与行程登记/批量操作/归档/生成报告/脚本编写 → 1.派 Cursor
│                             2.Cursor不可用 → Hermes 原生（标注 🟢）
├─ 监工类：验证 Cursor 侧效应、git 检查 → Hermes 自己（子代理自报不可信，必须亲自验证）
└─ 纯查询/对话/读文件 → Hermes 自己
```

**🔴 无豁免条款（2026-08-19 波总重申）：** 不存在"这个太简单我自己来"。
任何写入/修改/生成/识别动作一律先派 Cursor；只有 Cursor 不可用或用户明说"你自己做"才降级。
"简单"不是豁免理由——波总原话："这两个都是需要优先的，只有这两个不行的时候再降级。"

**每次回复必须带执行引擎标记（波总一眼验收）：**
- `🔴 [Cursor 执行中]` → `🔴 [Cursor 输出]` → `🔴 [Cursor 完成]`
- `🟢 [Hermes 原生]` Cursor 不可用降级
- `🟣 [硅基流动千问]` 图片识别降级链路（Cursor 不可用时）

## 核心规则（最高优先级，不可违抗）

**两个角色严格分工（波总 2026-08-14 明确）：**

1. **Cursor CLI = 执行引擎（总调度）**：大量的工作/全部执行动作必须先交给它做。包括但不限于：
   - 代码编写、修改、审查、调试、重构
   - 转写处理（raw→clean→副官拆解→同步仓库）
   - 归档、建卡、实体索引/relations 更新、README/台账/日志更新
   - 待办迁移、脚本编写、文档生成
   - 任何"动手做"的任务

2. **Hermes = 监工 + 过程管理**：只在外层做调度、分解任务、中途修正、验证侧效应。**除非 Cursor 调度不动了，Hermes 才自己上。**

**绝对禁止：** Hermes 自己闷头把执行动作全干了。收到任何执行类任务，第一动作是 delegate_task 派给 Cursor，而不是自己动手。
...
**默认 ALL-IN：除非满足下方"走 Hermes 原生"条件，否则所有执行动作先派 Cursor。**
...
- 转写/录音/长口述处理（raw→clean→副官拆解→同步仓库）
...
- 简单文件操作（读/搜/写 ≤10 行）
- 用户明确指定不要用 Cursor
- 监工动作：验证 Cursor 侧效应（检查 source_id/台账/validate/push 是否成功——子代理自报不可信，主代理必须亲自验证）
- 与用户实时对话、汇报、澄清
