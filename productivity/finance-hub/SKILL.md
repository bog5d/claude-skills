...
- **🔥 OCR 还款记录页 vs 余额页（🛑 2026-07-25 严重翻车）** — 度小满等平台的"在线还款记录"页列出了每笔还款金额（如¥1,489.71），OCR 可能误将其中一笔还款额识别为**当前总余额**。正确做法：余额页 = 总待还；还款记录页的单笔数字 ≠ 余额。波总说"杜小满已经全部还完了"是指**当月（7月26日到期）那笔已提前还清**，不代表未来月份（8/9/10/11/12月）没有剩余分期。必须区分「当期还清」vs「全部结清」。
- **📱 波总可直接复制粘贴文字** — 如果 OCR 识别存疑，波总可以直接从手机 App 复制文字粘贴过来。手机复制的文字 100% 准确，优先级高于 OCR 推理。收到粘贴文本后先逐行梳理、理解确认，再写入数据。
- **🧠 理解优先于执行（铁律）** — 收到财务数据后，**先复述理解**：「X月共N笔，分别是...，合计...」，等波总确认理解正确后，再去修改 debts.json。不要先改数据再问对不对——改错了波总骂人「你现在经常理解是错的」。翻车教训：度小满 OCR 读成¥1,489.71 后直接写入，波总被迫手动粘贴文字纠正。
- **⚠️ generate_starfire.py 运行需环境变量** — 在 Hermes profile sandbox 下 `~` 被重定向到 `~/.hermes/profiles/<name>/home/`，导致 REPO_DIR 路径解析失败。必须显式传入：`HOME=/Users/mac FINANCE_DIR=/Users/mac/.hermes/adjutant/finance python3 .../generate_starfire.py`。
