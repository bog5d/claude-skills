...
## 场景
DeepSeek 等主模型不支持原生 vision，图片识别走降级方案（Tesseract OCR）精度差。配置 `auxiliary.vision` 让截图自动路由到通义千问。
...
# provider: dashscope, model: qwen-vl-max, api_key: sk-*** ✓
```
...
curl -s https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions \
  ...
  -d '{"model":"qwen-vl-max","messages":[{"role":"user","content":"回复1+1"}]}'
```
...
- **长图测试**：又细又窄的长截图是通义千问的强项，Tesseract OCR 对此几乎无能
