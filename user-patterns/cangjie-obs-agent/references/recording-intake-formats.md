...
- **「利他 + 可合作」双维度（波总 2026-08-14 明确要求，人脉卡必带）**：
  - `## 利他清单（我们可为TA做的）` — 我们能帮对方的事，分 `### 公（业务/工作可帮）` 与 `### 私（个人可帮）` 两小节；开头写 `> 目标：全力把"利他"做到最好，先给后取。`
  - `## 可合作点（我们与TA的后续合作）` — 双方资源互补、可落地的合作方向，**重点提炼**（波总原话："除了能够利他的，还有重要的可合作点……可以重点说一下可合作的地方"）。
  - 两者**允许交集**（"能利他的也可能是可合作点"），不强行分开；每条约 1-2 行，标注具体抓手（项目名/金额/结构/时间窗）。实战样板：`人脉管理/李华清.md`、`人脉管理/张克成.md`（2026-08-14 建，均含双维度）。
