...
- **日程事件文件 status 枚举（2026-08-20 实测）**：`日程管理/YYYY/MM/*.md` 的 frontmatter `status` validate **只认 `tentative` / `confirmed`**，自定义值（如 `confirmed_by_screenshot`）报 `status 非法`。确认来源（截图/预订/用户明示）写进 `source` 字段或正文备注，不要发明枚举值。
...
- **cursor-agent 启动即崩排查（2026-08-20 实测+已修复）**：`cursor-agent -p --yolo` 秒退、日志只有 `/Users/mac/.openclaw/completions/openclaw.zsh:3803: command not found: compdef`。根因：`~/.zshrc` 第 5 行**无条件** source openclaw.zsh（130KB 补全脚本，其 3803 行用 `compdef`，非交互 zsh 未跑 compinit 时该函数未定义）；cursor-agent 是 bash wrapper→node（`file /opt/homebrew/bin/cursor-agent` 可见），内部 spawn 的 zsh 照读 ~/.zshrc——`zsh -f` 和 `ZDOTDIR=/tmp/empty` **都绕不过**（node 显式加载用户配置）。**修复（2026-08-20 波总授权"开干吧"后由 Hermes 落盘）**：~/.zshrc 的 source 行加交互守卫，且守卫内必须先 `autoload -Uz compinit && compinit` 再 source（否则交互 shell 仍报 compdef）：
```zsh
...
```
验证：`zsh -c 'echo ok'`（非交互干净）+ `zsh -i -c 'echo ok'`（交互干净）+ `cursor-agent -p --yolo "回复: TEST_OK"` 返回 TEST_OK。诊断链：`file /opt/homebrew/bin/cursor-agent` → `grep -rn openclaw ~/.zshrc` → `sed -n '3800,3805p' ~/.openclaw/completions/openclaw.zsh`。启动失败即按降级链 🟢 Hermes 原生跑完全流程（波总 2026-08-20 认可），不要死等 Cursor。**运行方式**：`-p` 模式必须前台/管道直连（timeout 600）；后台 `> log 2>&1` 常 0 字节日志秒退（proc exit code None），勿用后台重定向跑 cursor。
