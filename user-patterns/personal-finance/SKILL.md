...
| **分期贷款数据：先理解再改** | 度小满等分期贷款数据到后，先口头说一遍理解（本期还款 vs 总余额），等波总确认正确后再执行更新。不要拿到了就改。 | 本会话波总明确说"你先理解一下，理解一下再执行"——分期贷款的数据模型比花呗/拿去花复杂 |
...
14. **分类关键字自动学习** — 每次发现新的商户名模式（如"蹄花""龙森园""相思椒"），立即追加到 `expenses.json` 的 categories 关键字库。⚠️ **batch 导入后必须检查**：`grep '"其他"' expenses.json` 找出被归入"其他"的项 → recat 纠正 → 把商户关键字补入 `expenses.json` 的 categories 字段。已知常被误归"其他"的商户：DeepSeek API→经营-软件服务、阿里云→经营-软件服务、顺丰→日用-快递、享道出行→交通、安徽蒸小碗/XX蹄花/XX小碗→餐饮、直豆→娱乐、湖南计算智谷/XX物业→交通-停车、App Store/Apple Music→数码、合肥客运段→交通（火车票）、赵克祥HT0966→交通（出租车）、淘票票→娱乐、天天美食→餐饮美食、广八鲜碳炉鸡煲→餐饮美食、安庆市佳悦丽景酒店→旅行、上海顺途科技→交通（租车/出行）
...
21. **截图 OCR 管线（v4.3，2026-07-22 更新 — 代码化）** — ⚠️ **波总明确要求：不要靠自己 OCR！必须调用 `ocr_finance.py` 脚本。** 原因：大模型每次发挥不一致，达不到工业级稳定。正确做法：`source ~/.hermes/profiles/finance/.env && python3 ~/.hermes/adjutant/finance/scripts/ocr_finance.py <截图路径> [--creditor "平台名"]`。该脚本内置 SiliconFlow API 自检、连通性测试、结构化 OCR 提取、金额验证、更新 debts.json/transactions.json、同步 repo、游戏化引擎、Git push 全自动管线。API key 在 terminal 子进程中不可用，必须 `source .env` 先加载。旧引擎（vision_analyze/Apple Vision/Tesseract）已弃用，不要自动降级。详见 `debt-screenshot-auto-update` 技能的 v3.0 版本。
...
58. **⚠️ 分期贷款：OCR提取金额 = 本期还款，非总余额（🛑 2026-07-25 踩坑）** — 度小满、按期还等分期贷款与花呗/拿去花不同。OCR 提取的还款金额（如 ¥1,489.71）是**本期已还金额**（installment），不是债务总余额。`debts.json` 的 `amount` 应保持 **总剩余余额**（如 ¥29,500.84，含未来各期未还部分）。仅当贷款**全部结清**（所有期数还完 + 本金清零）才应移入 `cleared`。**典型错误**：看到度小满有一笔 ¥1,489.71 的还款记录，就把 debts.json 总额改成 ¥1,489.71，并说"度小满已全部结清"——正确理解是"7月26日那期已提前还清，但8月、9月、10月继续要还"。**铁律**：波总发来分期贷款截图/数据时，必须先确认数据类型——"本期已还款"还是"总剩余余额"；把理解说给波总听，等他确认后再改数据。
