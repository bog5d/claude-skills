...
echo "接盘完成。Skills: $(ls ~/.claude/skills/**/ 2>/dev/null | grep -c SKILL.md 2>/dev/null || echo '见仓库') 个"
```
...
# 拉取最新记忆
cd ~/.claude/memory && git pull origin master 2>/dev/null || true
```
...
# 推送记忆变更（如有新的上下文/决策记录）
cd ~/.claude/memory && git add -A && git commit -m "memory: $(date +%Y%m%d-%H%M%S)" && git push origin master
```
