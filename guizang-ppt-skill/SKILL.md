...
### Step 5 · 生成文件（关键：避开输出长度限制）

**硬性规则：PPTs 必须写入文件，不允许对话输出**

PPT 的 HTML 文件通常有 30-70KB，远超对话/Telegram 输出长度限制（约 2-4 万字）。输出会被截断，导致只输出半套 slides 或没有 CSS/JS。

**这是策略性问题，不是技术问题。** 不是"输出能被修得更大"，而是**工作流的正确做法就是搜索→存文件→生成→写文件，不让中间产物经过对话流**。

**决策树（每次生成前必须过一遍）：**

```
搜索数据量是否超过 2000 字？
  ├── 否 → 可以直接拼接写入（但也是写文件，不是对话输出）
  └── 是 → 绝对必须：
        1. 搜索数据先存为独立文件（如 research-data.json / research-data.txt）
        2. 单独读取文件内容拼接 PPT
        3. 最终 PPT 写入文件，对话只回复文件路径
```

**正确做法**：将模板和 slides 拼接后，通过 `terminal` 或 `write_file` 写入文件：

```bash
# 方案 A：Python 拼接后写文件（推荐）
cd ~/Desktop/rwa-ppt
python3 << 'PYEOF'
import os

# 1. 读取模板完整内容
with open(os.path.expanduser('~/.hermes/skills/guizang-ppt-skill/assets/template.html'), 'r', encoding='utf-8') as f:
    content = f.read()

# 2. 找到 SLIDES_HERE 分割点
idx = content.find('SLIDES_HERE')
head = content[:idx]
tail = content[idx+len('SLIDES_HERE'):]

# 3. 替换标题
head = head.replace('<title>[必填] 替换为 PPT 标题 · Deck Title</title>', '<title>你的标题</title>')

# 4. 替换主题色
head = head.replace(... 主题色代码 ...)

# 5. 拼接 slides 并写出
result = head + slides_content + tail
with open(os.path.expanduser('~/Desktop/rwa-ppt/index.html'), 'w', encoding='utf-8') as f:
    f.write(result)
PYEOF

# 方案 B：cp 模板后用 sed 替换
cp ~/.hermes/skills/guizang-ppt-skill/assets/template.html ~/Desktop/rwa-ppt/index.html
# 然后通过 patch 工具替换特定行/内容
```

**关键点**：
- 模板的 `template.html` 有 600+ 行、30KB，**绝对不能通过对话输出**——必须用终端直接读写文件
- `execute_code` 里的 `read_file` 可能返回截断内容（受 50KB stdout 限制），请用 terminal 的 python3 直接 `open().read()`
- 生成后底部有模板自带的 1 个示例 slide（被 `<!-- -->` 注释掉），需要手动删除
